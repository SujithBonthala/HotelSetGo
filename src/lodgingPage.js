...
function LodgingPage({setPrice}){
    ...
    const [room, setRoom] = useState({
      ...
      const Search =()=>
      {
          ...
          if((date3>=date4) && (date1<=date2))
          {
              axios.post("http://localhost:8000/lodgingPage", room).then((res)=>{
                console.log(res.data)
                rooms=res.data.room
                swal(res.data.message);
                if(rooms)
                {
                    document.getElementById('l8').innerHTML="Rs."+" "+rooms[0].price;
                    setPrice(rooms[0].price);
      ...
      {
          if (room.id) {
            axios.put("http://localhost:8000/lodgingPage", room).then((res)=>{
                swal(res.data.message)
                setRoom({id:""})
                rooms=[];
          ...
          {
              swal("Room of your choice is not found!!")
          }
      }
    ...
    return(
        <div className="full2">
            ...
            <form id='f1'>
                ...
                <input type="date" id='i1' name="from" value={room.from}  onChange={handleChange}/>
                <label id='l3'><b>Check-Out: </b></label>
                <input type="date" id='i2' name="to" value={room.to}  onChange={handleChange}/><br/><br/>
                ...
                <input type="text" name="number" id='i3' value={room.number} onChange={handleChange}/><br/><br/>
                <label for="cno" id='l6'><b>Contact No.: </b></label>
                <input type="text" name="cno" id='i4'/>
                <label id='l7'><b>Total Price: </b></label><label id='l8'><b> </b></label><br/><br/><br/><br/>
                <input type="button" value="Search Room" id='b1' onClick={Search}/>
                <input type="button" value="Confirm booking" id='b1' onClick={Confirm}/>
            </form>
            </div>
        </div>
    );
